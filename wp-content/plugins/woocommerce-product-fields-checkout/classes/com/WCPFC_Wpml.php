...
class WCPFC_Wpml
{
	...
	{
		if(!isset($_POST['wcpfc_wpml_language']) || !$this->wpml_is_active())
			return;
		
		global $sitepress;
		load_plugin_textdomain('woocommerce-product-fields-checkout', false, WCPFC_PLUGIN_LANG_PATH );
		$sitepress->switch_lang($_POST['wcpfc_wpml_language'], true);
	}
	...
	public function get_all_translation_ids($post_id, $post_type = "product")
	{
		...
		//if($post_type == "product")
		{
			...
			//wcpfc_var_dump($translations_result);
		}
	...
	{
		if(!class_exists('SitePress') || !isset($items_array))
			return $items_array;
		
		global $sitepress;
		$filtered_items_list = array();
		$items_array = !is_array($items_array) ? array($items_array) : $items_array;
		foreach($items_array as $item)	
		{
			$item_id = is_object($item) && method_exists($item,'get_id') ? $item->get_id() : $item;
	...
	public function get_current_language() //NO -> use get_current_locale()
	{
		if(!class_exists('SitePress'))
			return get_locale();
	...
	function translate_single_string($id, $text, $prefix = "wcpfc_label_"  )
	{
		if(!class_exists('SitePress'))
			return $text;
		
		$result = apply_filters( 'wpml_translate_single_string', $text, 'woocommerce-product-fields-checkout', 'wcpfc_'.$prefix.$id, ICL_LANGUAGE_CODE  );
			
		return $result != "" ? $result : $text;
	}
	function register_single_string($id, $text, $prefix = "wcpfc_label_" )
	{
		do_action( 'wpml_register_single_string', 'woocommerce-product-fields-checkout', 'wcpfc_'.$prefix.$id, $text);
	}
	...
	public function get_langauges_list()
	{
		 ...
		 */
		 ...
		 return !$this->wpml_is_active() || empty($langs) ? array(0 => array("id"=> 0, "code" => 0, "default_locale" => $this->get_default_locale(), "country_flag_url" => "none")) : $langs;
	}	
}
?>