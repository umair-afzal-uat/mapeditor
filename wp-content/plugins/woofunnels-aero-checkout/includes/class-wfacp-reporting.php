...
class WFACP_Reporting {
	...
	public function create_table() {
		...
		dbDelta( $creationSQL );
		update_option( 'wfacp_db_ver_2_0', gmdate( 'Y-m-d' ) );
	}
	...
	public function update_reporting_data_in_meta( $order, $posted_data ) {
		...
		$funnel_id = 0;
		$fid       = get_post_meta( $wfacp_id, '_bwf_in_funnel', true );
		if ( $fid > 0 ) {
			$funnel_id = $fid;
		}
	...
	public function get_ipn_gateways() {
		return apply_filters( 'wfacp_ipn_gateways_list', array( 'paypal', 'mollie_wc_gateway_ideal', 'mollie_wc_gateway_bancontact', 'mollie_wc_gateway_sofort' ) );
	}
}
