...
final class WFACP_Template_loader {
	...
	public function localize_page_builder_texts() {
		...
		foreach ( $get_all_opted_page_builders as $builder ) {
			...
			$plugin_string   = sprintf( __( 'This template needs %s plugin activated.', 'woofunnels-flex-funnels' ), esc_html( $page_builder['title'] ) );
			$button_text     = __( 'Activate the plugin', 'woofunnels-flex-funnels' );
			$no_install      = 'no';
			$title           = __( 'Unable to import template', 'woofunnels-flex-funnels' );
			$show_cancel_btn = 'yes';
			...
			if ( 'divi' === $builder ) {
				if ( 'activated' !== $theme_status && 'activate' === $plugin_status ) {
					...
					$button_text     = __( 'Close', 'woofunnels-flex-funnels' );
					$show_cancel_btn = 'no';
					$no_install      = 'yes';
				}
			} else if ( 'oxy' === $builder ) {
