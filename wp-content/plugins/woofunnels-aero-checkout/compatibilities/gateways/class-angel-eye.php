...
class WFACP_Compatibility_With_Angel_Eye {
	public function __construct() {
		...
		add_action( 'wfacp_after_form', [ $this, 'try_to_load_faster' ] );
...
//		VERSION_PFW
	...
	public function try_to_load_faster() {
		if ( ! class_exists( 'Angelleye_PayPal_Express_Checkout_Helper' ) ) {
			return '';
		}
		?>
        <script>
            window.addEventListener('bwf_checkout_load', function () {
                (function ($) {
                    let checkout_paypal_message = $("#wfacp_smart_button_paypal_express #checkout_paypal_message");
                    if (checkout_paypal_message.length > 0) {
                        $(document.body).trigger('updated_cart_totals');
                        checkout_paypal_message.on('DOMNodeInserted', function () {
                            $("wfacp_smart_buttons").removeClass('wfacp-dynamic-checkout-loading');
                            $("wfacp_smart_buttons").show();
                        });
                    }
                })(jQuery);
            });
