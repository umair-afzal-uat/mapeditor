		...
		foreach ( $wpsf_settings['sections'] as $key => $value ) {
			...
			foreach ( $value['fields'] as $field_key => $field_value ) {
				if ( isset( $field_value['id'] ) && $field_value['id'] == 'position' ) {
					$wpsf_settings['sections'][ $key ]['fields'][ $field_key ]['choices']['wfacp_after_wfacp_divider_shipping_end_field'] = "WooFunnels Checkout After Shipping Fields";
					$wpsf_settings['sections'][ $key ]['fields'][ $field_key ]['choices']['wfacp_after_wfacp_divider_billing_end_field']  = "WooFunnels Checkout After Billing Fields";
