...
class WFACP_Compatibility_With_Woo_PostNl {
	...
	public function display_field( $field, $key ) {
		...
		if ( ! $this->is_enabled() || empty( $key ) || 'wfacp_postnl_parcel' !== $key || ! $this->instance instanceof WCPN_Checkout ) {
			return '';
		}
	...
	public function add_internal() {
		if ( ! $this->is_enabled() || ! $this->instance instanceof WCPN_Checkout ) {
			return '';
		}
		?>
