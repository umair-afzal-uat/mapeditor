...
Place your license key here', 'woofunnels' ) . '" />' . "\n";
		...
		if ( $this->is_license_expire( $item ) ) {
			...
			$response_notice .= '<span class="below_input_message">' . __( sprintf( 'This license has expired. Login to <a target="_blank" href="%s">Your Account</a> and renew your license.', 'https://account.buildwoofunnels.com/' ), 'woofunnels' ) . '</span>';
	...
	public function column_product_expiry( $item ) {
		...
		if ( $this->is_license_expire( $item ) ) {
			$date        = new DateTime( $item['license_expiry']['expires'] );
			$date_string = $date->format( get_option( 'date_format' ) );

			return $date_string;
		} else {
			if ( "" == $item['existing_key']['expires'] ) {
	...
	public function column_activation_email( $item ) {
		$response    = '';
		$input_email = '<input name="license_keys[' . esc_attr( $item['product_file_path'] ) . '][email]" id="license_keys-' . esc_attr( $item['product_file_path'] ) . '" type="text" aria-required="true" value="' . $item['existing_key']['email'] . '" placeholder="' . esc_attr__( '
Place your license key here', 'woofunnels' ) . '" />' . "\n";

		if ( $this->is_license_expire( $item ) ) {
			$response .= $input_email;
		} else {
			if ( '' == $item['existing_key']['email'] ) {
				$response = $input_email;
			} else {
				$response = $item['existing_key']['email'];
			}
		}
